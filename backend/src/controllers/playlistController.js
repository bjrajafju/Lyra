...
const normalizePagination = (page, limit, defaultLimit = 20, maxLimit = 100) => {
    const parsedPage = Number.parseInt(page, 10);
    const parsedLimit = Number.parseInt(limit, 10);
    const safePage = Number.isNaN(parsedPage) || parsedPage < 1 ? 1 : parsedPage;
    const safeLimitRaw = Number.isNaN(parsedLimit) || parsedLimit < 1 ? defaultLimit : parsedLimit;
...
export const getPlaylists = async (req, res) => {
    ...
    const orderClause = sort === 'popular' ? 'song_count DESC, p.created_at DESC' : 'p.created_at DESC';
...
export const addSongToPlaylist = async (req, res) => {
    ...
    try {
        ...
        const songCheck = await query('SELECT id FROM songs WHERE id = $1 AND visibility = $2', [song_id, 'public']);
        if (songCheck.rows.length === 0) {
            return res.status(404).json({ message: 'Song not found or not public' });
        }
...
export const removeSongFromPlaylist = async (req, res) => {
    ...
    try {
        ...
        const deleted = await query('DELETE FROM playlist_songs WHERE playlist_id = $1 AND song_id = $2 RETURNING position', [id, songId]);
...
export const getPlaylistById = async (req, res) => {
    ...
    const { limit: safeLimit, offset } = normalizePagination(page, limit, 100, 500);
    try {
...
export const getPlaylistShareInfo = async (req, res) => {
    try {
        const playlist = await query('SELECT id, title, is_public FROM playlists WHERE id = $1', [req.params.id]);
        if (playlist.rows.length === 0) return res.status(404).json({ message: 'Playlist not found' });
        if (!playlist.rows[0].is_public) return res.status(403).json({ message: 'Only public playlists can be shared' });
...
export const reorderPlaylistSongs = async (req, res) => {
    ...
    try {
        ...
        const existingIds = existing.rows.map((row) => Number.parseInt(row.song_id, 10));
        ...
        if (hasInvalidIncoming) {
            return res.status(400).json({ message: 'song_ids must contain valid song ids' });
        }

        if (existingIds.length !== incomingIds.length) {
            return res.status(400).json({ message: 'song_ids must include all playlist songs exactly once' });
        }
        ...
        console.error(error);
        res.status(500).json({ message: 'Server error reordering playlist songs' });
    }
};
