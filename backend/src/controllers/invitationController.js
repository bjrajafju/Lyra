...
export const createInvitation = async (req, res) => {
    ...
    if (!inviteeId || !role) {
        return res
            .status(400)
            .json({ message: "Invitee ID and role are required" });
    }
    ...
    try {
        ...
        if (inviterRole === "editor" && role === "admin") {
            return res
                .status(403)
                .json({ message: "Editors cannot invite Admins" });
        }
        ...
        if (memberCheck.rows.length > 0) {
            return res
                .status(400)
                .json({ message: "User is already a member of this band" });
        }
...
export const respondToInvitation = async (req, res) => {
    ...
    try {
        ...
        await query("UPDATE invitations SET status = $1 WHERE id = $2", [
            status,
            invitationId,
        ]);
