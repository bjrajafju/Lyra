    ...
    try {
        ...
        if (req.files) {
            if (req.files.profile_image)
                profile_image = `/uploads/images/${req.files.profile_image[0].filename}`;
            if (req.files.banner_image)
                banner_image = `/uploads/images/${req.files.banner_image[0].filename}`;
        }
    ...
    try {
        ...
        if (req.files) {
            if (req.files.profile_image) {
                ...
                values.push(
                    `/uploads/images/${req.files.profile_image[0].filename}`,
                );
            }
            if (req.files.banner_image) {
                updateFields.push(`banner_image = $${paramIndex++}`);
                values.push(
                    `/uploads/images/${req.files.banner_image[0].filename}`,
                );
            }
        }
