...
export const updateWidget = async (req, res) => {
    ...
    try {
        ...
        if (settings !== undefined) { updateFields.push(`settings = $${pIndex++}`); params.push(JSON.stringify(settings)); }
        if (position !== undefined) { updateFields.push(`position = $${pIndex++}`); params.push(position); }
        ...
        if (result.rows.length === 0) return res.status(404).json({ message: 'Widget not found' });
...
export const deleteWidget = async (req, res) => {
    const { widgetId } = req.params;
    try {
        const result = await query('DELETE FROM band_widgets WHERE id = $1 RETURNING *', [widgetId]);
        if (result.rows.length === 0) return res.status(404).json({ message: 'Widget not found' });
...
export const reorderWidgets = async (req, res) => {
    ...
    if (!widgetOrders || !Array.isArray(widgetOrders)) {
        return res.status(400).json({ message: 'widgetOrders array is required' });
    }
