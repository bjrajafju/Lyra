...
export const searchAll = async (req, res) => {
    ...
    try {
        ...
        const songs = await query('SELECT s.*, b.name as band_name FROM songs s JOIN bands b ON s.band_id = b.id WHERE s.title ILIKE $1 AND s.status = $2 LIMIT 10', [searchStr, 'published']);
...
export const searchUsers = async (req, res) => {
    ...
    try {
        let sql = 'SELECT id, username, profile_picture FROM users WHERE username ILIKE $1';
        let params = [searchStr];

        if (excludeBandId) {
            sql += ' AND id NOT IN (SELECT user_id FROM band_members WHERE band_id = $2)';
        ...
        const usersSql = sql + ` LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
...
export const getDiscovery = async (req, res) => {
    try {
        ...
        const trending = await query('SELECT s.*, b.name as band_name FROM songs s JOIN bands b ON s.band_id = b.id WHERE s.status = $1 ORDER BY s.play_count DESC LIMIT 10', ['published']);
        // New releases
        const newReleases = await query('SELECT s.*, b.name as band_name FROM songs s JOIN bands b ON s.band_id = b.id WHERE s.status = $1 ORDER BY s.created_at DESC LIMIT 10', ['published']);
