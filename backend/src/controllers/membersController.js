...
export const updateMemberRole = async (req, res) => {
    ...
    if (req.user.id === parseInt(userId)) {
        return res
            .status(403)
            .json({ message: "You cannot edit your own role" });
    }
    ...
    try {
        ...
        if (inviterRole === "editor") {
            if (
                ...
                return res
                    .status(403)
                    .json({ message: "Editors can only manage member roles" });
            }
            if (role === "admin") {
                return res
                    .status(403)
                    .json({
                        message: "Editors cannot promote members to Admin",
                    });
            }
        }
    ...
    try {
        ...
        if (inviterRole === "editor") {
            if (currentRole !== "member") {
                return res
                    .status(403)
                    .json({ message: "Editors can only remove members" });
            }
        }
...
export const leaveBand = async (req, res) => {
    ...
    try {
        ...
        if (targetResult.rows.length === 0) {
            return res
                .status(404)
                .json({ message: "You are not a member of this band" });
        }
        ...
        if (currentRole === "admin") {
            ...
            if (parseInt(adminCount.rows[0].count) <= 1) {
                return res
                    .status(400)
                    .json({ message: "The last Admin cannot leave the band" });
            }
        }
