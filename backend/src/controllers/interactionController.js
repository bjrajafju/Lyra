...
export const toggleFollow = async (req, res) => {
    ...
    try {
        const table =
            followed_type === "band" ? "band_follows" : "user_follows";
        const id_col =
            followed_type === "band" ? "band_id" : "followed_user_id";

        const check = await query(
            `SELECT * FROM ${table} WHERE follower_id = $1 AND ${id_col} = $2`,
            [follower_id, followed_id],
        );
        if (check.rows.length > 0) {
            await query(
                `DELETE FROM ${table} WHERE follower_id = $1 AND ${id_col} = $2`,
                [follower_id, followed_id],
            );
            res.json({ message: "Unfollowed", following: false });
        } else {
            await query(
                `INSERT INTO ${table} (follower_id, ${id_col}) VALUES ($1, $2)`,
                [follower_id, followed_id],
            );
    ...
    try {
        ...
        if (band_id) {
            const followCheck = await query(
                "SELECT 1 FROM band_follows WHERE follower_id = $1 AND band_id = $2",
                [user_id, band_id],
            );
        ...
        if (target_user_id) {
            const followCheck = await query(
                "SELECT 1 FROM user_follows WHERE follower_id = $1 AND followed_user_id = $2",
