...
class _InviteMemberScreenState extends State<InviteMemberScreen> {
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Invite ${user['username']}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              const Text('Select a role for this user:', style: TextStyle(color: AppTheme.textSecondary)),
              const SizedBox(height: 24),
              _roleOption('member', 'Member', 'Can view dashboard and stats', selectedRole, (val) => setModalState(() => selectedRole = val)),
              _roleOption('editor', 'Editor', 'Can manage content and members', selectedRole, (val) => setModalState(() => selectedRole = val)),
              _roleOption('admin', 'Admin', 'Full control including deletion', selectedRole, (val) => setModalState(() => selectedRole = val)),
              const SizedBox(height: 24),
              ElevatedButton(
                onPressed: () => _sendInvitation(user['id'].toString(), selectedRole),
  ...
  Widget _roleOption(String value, String title, String subtitle, String selected, Function(String) onSelect) {
  ...
  Future<void> _sendInvitation(String userId, String role) async {
    ...
    try {
      await InvitationService.inviteUser(widget.bandId.toString(), userId, role);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Invitation sent successfully!'), backgroundColor: AppTheme.success),
        );
      }
    } catch (e) {
      if (mounted) {
